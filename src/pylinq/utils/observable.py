...
def must_have_event(func):
    def decorated(observable, event_name, *args,  **kwargs):
        if event_name not in observable.events:
            raise UnsupportedEventException('This observable does not support event "%s"' % event_name)
...
class Observable(object):
    ...
    def bind(self, event_name, event_handler, *args):
        if not callable(event_handler):
            raise HandlerMustBeCallableException('Event handler must be callable')
        self.events[event_name].append((event_handler, args))
    
    @must_have_event
    def trigger(self, event_name, *args):
        listeners  = self.events[event_name]
        for handler, extra_args in listeners:
